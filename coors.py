...
def BLH2NEU(station):
    ...
    :param station: BLH in rad
    :param obj: BLH in rad
    :return:
    '''
    # print(station, obj)
    rotation = np.zeros([3, 3])
    B, L = station[0], station[1]
    rotation[0, 0] = -np.sin(L)
    rotation[0, 1] = -np.sin(B) * np.cos(L)
    rotation[0, 2] = np.cos(B) * np.cos(L)

    rotation[1, 0] = np.cos(L)
    rotation[1, 1] = -np.sin(B) * np.sin(L)
    rotation[1, 2] = np.cos(B) * np.sin(L)

    rotation[2, 0] = 0
    rotation[2, 1] = np.cos(B)
